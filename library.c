...
PHP_REDIS_API int redis_sock_disconnect(RedisSock *redis_sock TSRMLS_DC)
{
    ...
    if (redis_sock->stream != NULL) {
			if (!redis_sock->persistent) {
				char* response;
				int response_len;
				char* quit = "*1" _NL "$4" _NL "QUIT" _NL;

				redis_sock_write(redis_sock, quit, sizeof(quit) + 6 TSRMLS_CC);
// normal redis servers says +OK to the above command, default nutcracker doesn't
